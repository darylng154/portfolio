...
{
	// create the socket
	if ((server->socketNum = socket(AF_INET6,SOCK_DGRAM,0)) < 0)
	{
	...
	// set up the socket
	memset(&(server->remote), 0, sizeof(struct sockaddr_in6));
	(server->remote).sin6_family = AF_INET6;    		// internet (IPv6 or IPv4) family
	(server->remote).sin6_addr = in6addr_any ;  		// use any local IP address
	(server->remote).sin6_port = htons(serverPort);   // if 0 = os picks 

	// bind the name (address) to a port
	if (bind(server->socketNum,(struct sockaddr *) &(server->remote), sizeof((server->remote))) < 0)
	{
	...
	/* Get the port number */
	server->length = sizeof(server->remote);
	getsockname(server->socketNum,(struct sockaddr *) &(server->remote),  (socklen_t *) &(server->length));
	printf("Server using Port #: %d\n", ntohs((server->remote).sin6_port));
