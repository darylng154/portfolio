...
Description:  
    ...
    Compile Options:    java lab4 file1 [script] [v] [m]
     ...
     ex. java lab4 lab4_test2.asm lab4_test2.script - -

Comments:
    With how my program was structured, I couldn't update the registers properly for the
    branch delays. Programs that rely on register values to branch will not work.
*/
...
public class lab4
{
    ...
    */
    ...
    public static void simulate(String[] args, HashMap<String, Integer> regMap, HashMap<String, Data> instrMap, HashMap<String, Integer> labelMap, List<String> assembly, List<int[]> dec, int[] dataMem, int[] regData, Pipeline[] pipes)
    {
        Scanner user = null;
        char command = '\0';
        ...
        while (run == true)
        {
            ...
            if (!userIn.isEmpty())
            {
                ...
                aCommand(args, regMap, instrMap, labelMap, assembly, dec, dataMem, regData, command, argNum, pipes);
            }
        }
    }
    ...
    public static void aCommand(String[] args, HashMap<String, Integer> regMap, HashMap<String, Data> instrMap, HashMap<String, Integer> labelMap,
                                List<String> assembly, List<int[]> dec, int[] dataMem, int[] regData, char command, int[] argNum, Pipeline[] pipes)
    {
        switch (command)
        {
            case 'h':   //display help
            ...
            case 's':   //step thru mips
                if(argNum[0] == 0 && argNum[1] == 0)    //single step   argNum[] == arguments for commands
                {
            ...
            //Lab 4 (incomplete)
            case 's':   //1 clock cycle thru mips
                if (argNum[0] == 0 && argNum[1] == 0)    //single cycle   argNum[] == arguments for commands
                {
                    aCycle(args, instrMap, labelMap, assembly, dec, dataMem, regData, argNum, pipes, regMap);
                    printPipes(args, pipes);
                }
                else    //multi cycles argNum[0] times
                {
                    for (int i = 0; i < argNum[0]; i++)
                    {
                        aCycle(args, instrMap, labelMap, assembly, dec, dataMem, regData, argNum, pipes, regMap);
            ...
            case 'r':   //run all assembly
                while (pc < assembly.size() || pipes[IF].instr != null)  //need to clear after last inst
                {
                    aCycle(args, instrMap, labelMap, assembly, dec, dataMem, regData, argNum, pipes, regMap);
                }
                //numCycles++;    //for the last instr where pc < assembly.size()
                System.out.println("\nProgram complete");
                System.out.format("CPI = %.2f        Cycles = %-4d     Instructions = %-4d \n\n", (float) numCycles / numInstr, numCycles, numInstr);
                break;
    ...
    public static int numCycles, numInstr = 0;
    public static final int IF = 0;
    public static final int ID = 1;
    public static final int EX = 2;
    public static final int WB = 3;

    public static void aCycle(String[] args, HashMap<String, Data> instrMap, HashMap<String, Integer> labelMap, List<String> assembly,
                              List<int[]> dec, int[] dataMem, int[] regData, int[] argNum, Pipeline[] pipes, HashMap<String, Integer> regMap)
    {
        numCycles++;
        ...
        if (args.length >= vArg && pc < assembly.size())
        {
            if (args[vArg - 1].charAt(0) == 'v')
            {
                System.out.println(pc + " " + assembly.get(pc));
        ...
        //Use-After-Load
        if (pipes[ID].instr.equals("lw") && (dec.get(pipes[ID].pc)[rt] == dec.get(pipes[IF].pc)[rs] || dec.get(pipes[ID].pc)[rt] == dec.get(pipes[IF].pc)[rt])
                && dec.get(pipes[ID].pc)[rt] != 0 && !(pipes[ID].instr.equals("bne") || pipes[ID].instr.equals("beq")) && !(pipes[EX].instr.equals("bne") || pipes[EX].instr.equals("beq")))
        {
            for (int i = pipes.length - 1; i > ID; i--)
            {
                pipes[i].instr = pipes[i - 1].instr;
                pipes[i].pc = pipes[i - 1].pc;
            }
            //stall
            pipes[ID].instr = "stall";
            pipes[ID].pc = 0;
        }
        else
        {
            //System.out.println("PC = " + pc);
            //Fetch & Add to Pipeline
            if (pipes[IF].instr != null && !(pipes[IF].instr.isEmpty()))
            {
        ...
        if (!(pipes[ID].isEmpty()))
        {
            if ((curData = instrMap.get(pipes[ID].instr)) != null)
            {
                instr = pipes[ID].instr;

                if (instr.equals("jr") || instr.equals("jal") || instr.equals("j"))
                {
                    pipes[IF].squash();
                }
            }
        ...
        // Exec, MEM, WB
        if(!(pipes[ID].isEmpty()) && pipes[ID].instr != null)
        {
           numInstr++;
           //System.out.println("instr++ = " + numInstr);
        }

        if(!(pipes[WB].isEmpty()))
        {
            if(pipes[WB].isBranch() && pipes[WB].pc == pc)
            {
                for(int i = 0; i < pipes.length-1; i++)
                {
                    pipes[i].instr = "squash";
                }
            }
        }
        ...
        {
            //numInstr++;
            if ((curData = instrMap.get(pipes[EX].instr)) != null)
            {
                curDec = dec.get(pipes[EX].pc);
                instr = pipes[EX].instr;
                ...
                switch (curData.format)
                {
                    ...
                    case 'i':
                        ...
                        {
                            if (regData[curDec[rs]] == regData[curDec[rt]])
                                pipes[EX].pc = curDec[rd] + pipes[EX].pc + 1;
                        }
                        else if (instr.equals("bne"))
                        {
                            if (regData[curDec[rs]] != regData[curDec[rt]])
    ...
    public static void printPipes(String[] args, Pipeline[] pipes)
    {
        System.out.println("\npc\t\tif/id  id/exe  exe/mem  mem/wb");
        System.out.format("%-7s %-6s %-7s %-8s %-7s \n", pc, pipes[IF].instr, pipes[ID].instr, pipes[EX].instr, pipes[WB].instr);

        if (args.length >= vArg)
        {
            if (args[vArg - 1].charAt(0) == 'v')
            {
                System.out.format("%-7d %-6d %-7d %-8d %-7d \n", pc, pipes[IF].pc, pipes[ID].pc, pipes[EX].pc, pipes[WB].pc);
            }
        }
    ...
    public static boolean isPCAtLabel(HashMap<String, Integer> labelMap, Pipeline pipe)
    {
        boolean is = false;
        for (int value: labelMap.values())
        {
            if(pipe.pc == value)
                return is;
        }
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        //simulate(args, regMap, instrMap, labelMap, assembly, dec, dataMem, regData);
        ...
        //Pipelines                         //        [     0,      1,      2,      3 ]
        Pipeline[] pipes = new Pipeline[]   //pipes = [ if/id, id/exe, exe/mem, mem/wb]
                {new Pipeline(), new Pipeline(), new Pipeline(), new Pipeline()};

        printPipes(args, pipes);
        //System.out.println("pipesEmpty? = " + isEmpty(pipes));

        simulate(args, regMap, instrMap, labelMap, assembly, dec, dataMem, regData, pipes);
